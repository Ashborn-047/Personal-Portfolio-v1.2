...
export default function ProjectCard({project,index,onOpen,isMobile}){
  ...
  const descColor = isDusk ? "rgba(138,154,170,.75)" : "rgba(71,85,105,.85)";
  ...
  const visualBlock = (
      <div style={{borderRight:isMobile?"none":(isEven ? `1px solid rgba(${baseColor},.05)` : "none"),borderLeft:isMobile?"none":(!isEven ? `1px solid rgba(${baseColor},.05)` : "none"),borderBottom:isMobile?`1px solid rgba(${baseColor},.05)`:"none",position:"relative",overflow:"hidden",height:isMobile?180:"auto"}}>
  ...
  const textBlock = (
      <div style={{padding:isMobile?"18px 16px 22px":"28px 32px",display:"flex",flexDirection:"column"}}>
        ...
        <h2 style={{fontFamily:"Georgia,serif",fontSize:isMobile?19:24,fontWeight:400,color:textColor,marginBottom:8,letterSpacing:"-0.01em",lineHeight:1.2}}>
          {project.title}
        </h2>
        <p style={{fontSize:12,lineHeight:1.7,color:descColor,marginBottom:14,flex:1}}>
          {project.description}
        </p>
  ...
  return(
    <div
      ...
      style={{
        ...
        border:`1px solid ${hov?`rgba(${rgb},.3)`:`rgba(${baseColor},.05)`}`,
        background:hov?`rgba(${rgb},.022)`:`rgba(${baseColor},.012)`,
    ...
    >
      ...
      <div style={{position:"absolute",bottom:14,right: isEven ? 16 : undefined, left: isEven ? undefined : 16, fontSize:10,color:`rgba(${baseColor},.06)`,letterSpacing:"0.1em",fontFamily:"'Courier New',monospace"}}>
        0{index+1}
      </div>
    </div>
  );
