...
export const DuskHeroVisual = () => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Gradient Sky: Deep Indigo -> Violet
      const gradient = ctx.createLinearGradient(0, 0, 0, height);
      gradient.addColorStop(0, '#0A0A12'); // Midnight
      gradient.addColorStop(0.6, '#130E1B'); // Deep Purple
      gradient.addColorStop(1, '#1A1025'); // Bottom
      ctx.fillStyle = gradient;
      ctx.fillRect(0, 0, width, height);

      // Enhanced Cosmic Bloom - Multiple layers
      // Primary cosmic bloom (center-right)
      const bloom1 = ctx.createRadialGradient(width * 0.6, height * 0.35, 0, width * 0.6, height * 0.35, height * 0.5);
      bloom1.addColorStop(0, 'rgba(176, 106, 255, 0.15)'); // Brighter purple
      bloom1.addColorStop(0.4, 'rgba(138, 107, 255, 0.08)');
      bloom1.addColorStop(1, 'transparent');
      ctx.fillStyle = bloom1;
      ctx.fillRect(0, 0, width, height);

      // Secondary cosmic bloom (upper-left)
      const bloom2 = ctx.createRadialGradient(width * 0.2, height * 0.25, 0, width * 0.2, height * 0.25, height * 0.4);
      bloom2.addColorStop(0, 'rgba(138, 107, 255, 0.12)');
      bloom2.addColorStop(0.5, 'rgba(106, 75, 200, 0.05)');
      bloom2.addColorStop(1, 'transparent');
      ctx.fillStyle = bloom2;
      ctx.fillRect(0, 0, width, height);

      // Nebula streaks - atmospheric haze
      const nebula = ctx.createLinearGradient(0, height * 0.3, width, height * 0.7);
      nebula.addColorStop(0, 'rgba(138, 107, 255, 0.03)');
      nebula.addColorStop(0.5, 'rgba(176, 106, 255, 0.05)');
      nebula.addColorStop(1, 'rgba(138, 107, 255, 0.02)');
      ...
      vignette.addColorStop(1, 'rgba(10, 10, 18, 0.8)');
